...
### 你目前的狀況（2026-06-26 初測）
| 模型 | 準確率 | baseline | 判決 |
|------|--------|----------|------|
| fib_channel/SOL | 0.950 | 0.961 | ❌ 比不做還差（+1 只有 10 筆） |
| smc/ETH | 0.657 | 0.759 | ❌ 比不做還差 |

### ⭐ 2026-07-04 嚴格重測（修好 vol_z 洩漏 + scale_pos_weight 後）

用**線上 8 幣籃子的真實配置**（smc_structure/4h/8 幣）做無洩漏對照：每個幣時間軸
切半，前半只訓練 ML、後半（模型沒看過）才跑「有 ML vs 無 ML」的 walk-forward。

**訓練品質**：772 筆池化樣本，CV 準確率 0.548（baseline 0.571）、**AUC 0.557**（≈擲硬幣）。
特徵重要度平均分散在 er/atr_pct/atr/adx/vol_z，沒有任何強訊號。

**後半 OOS 對照**：
| | 交易 | 勝率 | 期望 | 信賴下界 | PF |
|---|---|---|---|---|---|
| 無 ML | 506 | 35.8% | +3.30 | **+0.65 ✅** | 1.24 |
| ML thr=0.50 | 279 | 36.2% | +1.85 | −1.68 ❌ | 1.13 |
| ML thr=0.55 | 231 | 34.6% | +1.08 | −3.04 ❌ | 1.07 |

**兩個定論**：
1. **籃子策略本身有統計顯著 OOS edge**（無 ML 信賴下界 +0.65 > 0）——池化 506 筆交易站穩。
2. **ML 過濾層有害，維持關閉**（`ml_filter: false`）。AUC 0.557 的模型≈隨機丟掉獲利交易，
   砍 45% 交易反而拉低期望、且樣本變少讓信賴區間變寬 → 顯著性被打成負。

**根因**：泛用波動度特徵無法預測「結構破壞訊號」的成敗。要 ML 有用，得換成**結構特徵**
（FVG 缺口大小、到 swing 距離、BOS 前動能），不是這些市場狀態指標。這不是 bug 造成的
假象——是修好 bug 後、用嚴格無洩漏方法證實的：特徵真的沒有預測力。

→ 結論：**先別再投資 ML 過濾層**；策略 edge 已經是真的，重心該放在維持籃子樣本累積。

（無洩漏對照的可重現腳本：backtester/tournament 的 `ml_model`/`ml_threshold` 參數；
單元測試見 tests/test_backtester.py::TestMlGateInBacktest、tests/test_tournament.py。）
