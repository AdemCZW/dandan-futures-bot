...
## 修訂記錄：2026-07-22 全自動核准+執行

使用者明確要求「全自動：AI 自己核准+下單」，取消人工核准這一關。這是本文件唯一標記過的
「不可退讓原則」（見下方第 2 條）被主動改動，故留痕說明：

- **狀態機本身不變**：`approved → placed` 這條轉移規則沒有放寬，`place_entry` 仍然要求
  `approved` 狀態才能執行。改變的是「誰呼叫 `approve()`」——新增 `ai_desk/auto.py`
  的 `run_auto_cycle()`，在風控放行提案的當下自動呼叫 `approve()` 再立刻 `place_entry()`，
  取代原本「等人類點按鈕」那一步。核准這個狀態轉移的**紀錄與稽核性質不變**，只是觸發者
  從人變成程式。
- **開關預設關閉**：`AI_DESK_AUTO_APPROVE`（預設 `false`）。沒設就是原本人工核准的行為，
  完全不影響既有手動流程；這是使用者主動選擇開啟的行為，非預設。
- **已知現況（決策當下的真實依據）**：截至本次修訂，累計 4 筆提案、1 勝 2 敗、
  紙上淨損 −36.73 USDT，樣本數遠低於本系統其餘策略要求的 bootstrap 信賴下界驗證門檻
  （原 rollout 表 Phase 3 明訂要「累積數週已核准+執行的測試網樣本」才考慮放寬人工核准）。
  使用者在知悉此現況與統計上尚未驗證的情況下，仍選擇现在開啟全自動——僅限 **Testnet
  虛擬資金**，下檔風險上限就是測試網餘額歸零，非真實財務損失。
- **其餘硬規則全部維持**（白名單、testnet 驗證、每幣一倉、絕不裸倉、reduceOnly 平倉、
  confidence 不參與倉位計算）——這次修訂只拿掉人工核准，不放寬任何其他防線。
