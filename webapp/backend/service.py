...
_LIVE_RECENT_TRADES_LIMIT = int(os.getenv("LIVE_RECENT_TRADES_LIMIT", "12"))
_LIVE_STATS_TRADES_LIMIT = int(os.getenv("LIVE_STATS_TRADES_LIMIT", "500"))
_LIVE_STATUS_CACHE_TTL = float(os.getenv("LIVE_STATUS_CACHE_TTL", "5"))
_LIVE_STATS_CACHE_TTL = float(os.getenv("LIVE_STATS_CACHE_TTL", "20"))
_LIVE_STATUS_FETCH_PRICE = os.getenv("LIVE_STATUS_FETCH_PRICE", "0").lower() in ("1", "true", "yes")

_live_status_cache: dict[tuple[str, str], tuple[float, dict]] = {}
_live_stats_cache: dict[tuple[str, str], tuple[float, dict]] = {}
...
def live_status(state_path: str = None, railway_url: str | None = None) -> dict:
    ...
    cache_key = (state_path or "", ru or "")
    now_ts = time.time()
    cached = _live_status_cache.get(cache_key)
    if cached and (now_ts - cached[0]) < _LIVE_STATUS_CACHE_TTL:
        return cached[1]
    import json
    ...
    if _LIVE_STATUS_FETCH_PRICE:
        try:                               # 可選：抓即時現價（公開行情、免金鑰）
            from core.market_analyst import make_client, fetch_klines
    ...
    # 近期成交（前端顯示用）+ 統計樣本（累計損益統計用）
    if ru and mode == "futures":
        recent = _fetch_railway_trades(limit=_LIVE_RECENT_TRADES_LIMIT, mode=trades_mode, base_url=ru)
        stats_cache_key = (ru, trades_mode)
        stats_cached = _live_stats_cache.get(stats_cache_key)
        if stats_cached and (now_ts - stats_cached[0]) < _LIVE_STATS_CACHE_TTL:
            ...
            _live_stats_cache[stats_cache_key] = (now_ts, stats_payload)
    else:
        recent = read_trades(limit=_LIVE_RECENT_TRADES_LIMIT, mode=trades_mode, db_path="trades.db")
        stats_cache_key = ("local", trades_mode)
        stats_cached = _live_stats_cache.get(stats_cache_key)
        if stats_cached and (now_ts - stats_cached[0]) < _LIVE_STATS_CACHE_TTL:
            stats_payload = stats_cached[1]
        else:
    ...
    out = {
        ...
        "realized_pnl": stats_payload["realized_pnl"],
        "total_trades": stats_payload["total_trades"],
        "win_trades": stats_payload["win_trades"],
        "max_drawdown_pct": stats_payload["max_drawdown_pct"],
        "sharpe": stats_payload["sharpe"],
        "long_trades": stats_payload["long_trades"], "long_wins": stats_payload["long_wins"],
        "long_pnl": stats_payload["long_pnl"],
        "short_trades": stats_payload["short_trades"], "short_wins": stats_payload["short_wins"],
        "short_pnl": stats_payload["short_pnl"],
        "updated_at": updated, "age_seconds": age, "poll": st.get("poll"),
        "last_decision": st.get("last_decision"),
        "recent_trades": recent,
    }
    _live_status_cache[cache_key] = (now_ts, out)
    return out
